...
async def create_appointment_from_ai(
    ...
    """
    Formato: scheduling:ID:YYYY-MM-DD:HHMM:NOME|CEP_ou_sem_cep|campo1=valor1|campo2=valor2
    Exemplos:
      scheduling:1:2026-07-24:1130:Gabriel Henrique|74948180|cpf=71164980106
      scheduling:2:2026-08-04:0900:Maria Silva|sem_cep
    """
    try:
        if not handoff_reason.startswith("scheduling:"):
            return None, AppointmentError("invalid_format", "Não começa com 'scheduling:'")

        # Split máximo em 5 partes para preservar o bloco de campos extras
        parts = handoff_reason.split(":", 4)
        if len(parts) < 5:
            return None, AppointmentError(
                "invalid_format", f"Partes insuficientes: {handoff_reason}"
            )

        _, service_id_str, date_str, time_hhmm, last_segment = parts
        service_id = int(service_id_str.strip())
        time_hhmm  = time_hhmm.strip()

        # Parse do último segmento — separado por |
        # Formato: NOME|CEP|campo1=valor1|campo2=valor2
        last_parts    = [p.strip() for p in last_segment.split("|")]
        customer_name = last_parts[0] if len(last_parts) > 0 else ""
        cep_raw       = last_parts[1] if len(last_parts) > 1 else "sem_cep"
        cep           = "" if cep_raw.lower() in ("sem_cep", "none", "") else cep_raw

        # Campos extras: campo=valor
        extra_fields: dict = {}
        for part in last_parts[2:]:
            if "=" in part:
                k, _, v = part.partition("=")
                extra_fields[k.strip()] = v.strip()

        # Parse hora — aceita HHMM ou HH:MM
        ...
        if service.required_fields:
            for field in service.required_fields:
                ...
                if required and key and key not in extra_fields:
                    label = field.get("label", key)
                    return None, AppointmentError(
                        "missing_field",
                        f"Campo obrigatório não informado: {label}"
                    )
        ...
        if weekday not in available_weekdays:
            dias = ", ".join(WEEKDAY_PT[w] for w in sorted(available_weekdays))
            return None, AppointmentError(
                "wrong_weekday",
        ...
        if time_str not in slots:
            proximos = slots[:5] if slots else []
            return None, AppointmentError(
                "slot_unavailable",
                f"Horário {time_str} indisponível em {target_date.strftime('%d/%m')}. "
                f"Disponíveis: {', '.join(proximos) if proximos else 'nenhum'}"
            )
        ...
        if service.location_enabled:
            if service.location_lat and service.location_lng:
                ...
                if client_lat is None:
                    return None, AppointmentError(
                        "cep_invalid",
                        f"CEP '{cep}' não encontrado. Confira se está correto (somente números)."
                    )
                ...
                log.info("[SCHEDULING] CEP %s → %.1fkm (limite: %skm)", cep, distance_km, service.location_radius_km)

                if distance_km > service.location_radius_km:
                    return None, AppointmentError(
                        "outside_radius",
                        f"Seu CEP está a {distance_km:.1f}km. "
        ...
        appt = Appointment(
            ...
            extra_fields=extra_fields  # CPF e outros campos extras
        )
        db.add(appt)
        db.flush()
        db.add(AppointmentHistory(
            ...
            changed_by="ia",
            action="created",
            notes=f"Via WhatsApp. CEP:{cep or 'não exigido'}. Extras:{extra_fields}. Status:{status.value}"
        ))
        db.commit()
        db.refresh(appt)

        log.info(
            "[SCHEDULING] #%s criado: %s — %s %s %s (%s) extras=%s",
            appt.id, customer_name.strip(), service.name, date_str, time_str, status.value, extra_fields
        )
        return appt, None
